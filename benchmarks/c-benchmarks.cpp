...
static void BM_CreateListMessage_finalized(benchmark::State &state) {
  if (VERBOSE)
    printf("BEGIN BM_CreateListMessage_finalized\n");
  ...
  pn_data_put_list(body);
  pn_data_enter(body);
  int64_t batch = state.max_iterations;
  if(state.KeepRunningBatch(batch)) {
  ...
  if (VERBOSE)
    printf("END BM_CreateListMessage_finalized\n");
}

BENCHMARK(BM_CreateListMessage_finalized);
